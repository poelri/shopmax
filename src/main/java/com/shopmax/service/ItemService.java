...
public class ItemService {
    ...
    public Long saveItem(ItemFormDto itemFormDto,
        ...
        //1. 상품 등록 (insert)
        Item item = itemFormDto.createItem(); //dto -> entitiy
    ...
    // 상품 가져오기
    @Transactional(readOnly = true) // 트랜잭션 읽기 전용 ( 변경 감지 수행X ) -> 성능향상
    public ItemFormDto getItemDtl(Long itemId){
        ...
        for (ItemImg itemImg : itemImgList) {
            ...
            itemImgDtoList.add(itemImgDto); // dto 객체로 바꾼 객체를 리스트에 넣어준다
        }

        // 2. item 테이블에 있는 데이터를 가져온다.
        // 프라이머리키로 가져올수있다.
        Item item = itemRepository.findById(itemId).orElseThrow(EntityNotFoundException::new); // Throw랑 세트임
    ...
    public Long updateItem(ItemFormDto itemFormDto,
        ...
        // 변경감지 기능으로 인해 트랜잭션 커밋 시점에 엔티티와 DB에 저장된 갑시
        // 다른 내용은 JPA에서 update 해준다.
    ...
    public Page<MainItemDto> getMainItemPage(ItemSearchDto itemSearchDto, Pageable pageable){
        Page<MainItemDto> mainItemDtoPage =
