...
public class OrderService {
    ...
    public Long order(OrderDto orderDto, String email){
        ...
        // 단방향 일때는 자식 엔티티 객체가 참조하는 부모 객체를 넣은 후 save
    ...
    public Page<OrderHistDto> getOrderList(String email, Pageable pageable) {
        ...
        // 3. 주문리스트 orders를 순회하면서 컨트롤러에 전달한 DTO(OrderHistDto)를 생성
    ...
    // 어노테이션은 읽기 전용 트랜잭션을 정의합니다.
    //readOnly = true로 설정되면, 해당 메서드 내에서는 데이터베이스의 상태를 수정하지 않는 읽기 작업
    @Transactional(readOnly = true)
    public boolean validateOrder(Long orderId, String email){
        ...
        if(!StringUtils.equals(cutMember.getEmail(), saveMember.getEmail())){
            return false;
        }
        return true;
    }
